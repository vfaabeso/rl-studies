class Maze:
    def __init__(self, width: int, height: int, start: tuple, goal: tuple, walls: list):
        ...
        self.start = self.coords_to_state(start)
        self.goal = self.coords_to_state(goal)
        self.walls = [self.coords_to_state(c) for c in walls]
        self.terminals = self.walls + [self.goal]
        # aside from normal cells
        self.special_cells = self.terminals + [self.start]
    ...
    def step(self, action: int) -> tuple:
        x, y = self.state_to_coords(self.current_state)
        if      action==0: y-=1
        elif    action==1: x+=1
        elif    action==2: y+=1
        elif    action==3: x-=1
        ...
        if x>=0 and x<self.width and y>=0 and y<self.height:
            # if hit the wall
            new_state = self.coords_to_state((x, y))
    ...
    # helper functions
    def coords_to_state(self, coords: tuple) -> int:
        return coords[1] * self.width + coords[0]

    def state_to_coords(self, state: int) -> tuple:
        x = state % self.width
        y = state // self.width
        return (x, y)
